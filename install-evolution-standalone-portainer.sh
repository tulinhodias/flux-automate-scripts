...
# ============================================================
# FLUX AUTOMATE - Evolution API Standalone + Portainer
# Instala: Docker + PostgreSQL + Redis + Evolution + Portainer
...
echo -e "${GREEN}"
echo "=============================================="
echo "  FLUX AUTOMATE - Evolution + Portainer        "
echo "  Evolution API + PostgreSQL + Redis           "
echo "       + Portainer + Nginx + SSL               "
echo "=============================================="
echo -e "${NC}"
...
if [ -z "$EVOLUTION_HOST" ] || [ -z "$PORTAINER_HOST" ] || [ -z "$EMAIL" ]; then
    print_error "Argumentos obrigatorios faltando!"
    echo ""
    echo "Uso:"
    echo "  bash install-evolution-standalone-portainer.sh \\"
...
mkdir -p /opt/flux-evolution/{postgres,redis,evolution,portainer}
...
# FLUX AUTOMATE - Evolution Standalone + Portainer
# Gerado em: $(date)
...
services:
  ...
  portainer:
    ...
    container_name: fluxevo_portainer
    restart: unless-stopped
    volumes:
      - /var/run/docker.sock:/var/run/docker.sock
      - /opt/flux-evolution/portainer:/data
    ports:
      - "9000:9000"
    networks:
      - flux_evo_network
...
all_ok=true
for svc in fluxevo_postgres fluxevo_redis fluxevo_evolution fluxevo_portainer; do
...
if [ "$all_ok" = true ]; then
    echo -e "${GREEN}"
    echo "=============================================="
    echo "   EVOLUTION + PORTAINER - INSTALADOS!         "
    echo "=============================================="
    echo -e "${NC}"
    echo ""
    echo -e "${BLUE}Acesse seus servicos:${NC}"
    echo "  Evolution:  https://${EVOLUTION_HOST}"
